...
/-- If `mu` has no atom at `x`, the downward jump contributed by a possible
atom of `nu` makes the signed cumulative value no larger than its left
limit. -/
theorem signedCumulativeLeLeftOfAtomlessAt
