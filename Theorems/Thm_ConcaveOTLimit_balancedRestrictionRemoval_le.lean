...
/-- A mass-balanced scaled removal from two disjoint blocks is dominated by
the original finite measure when both scaling coefficients are at most one. -/
theorem balancedRestrictionRemoval_le
