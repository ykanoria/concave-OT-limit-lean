...
/-- A forward arch in a full-mass monotone support certifies that its source
cumulative level is minimal throughout the open arch and belongs to the
completed cumulative graph at the target endpoint. -/
theorem monotoneArchCumulativeCertificate
    {mu nu : FiniteMeasure Real}
    (gamma : FiniteCoupling mu nu)
