...
/-- A backward arch and a strictly forward arch crossing the same cut cannot
satisfy Juillet's pairwise monotone-arch conditions. -/
theorem oppositeMonotoneArchesCannotCrossCut
    {x y x' y' t : Real}
    (hBackward : y <= t /\ t < x)
    (hForward : x' < t /\ t < y')
