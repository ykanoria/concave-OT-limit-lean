...
/-- A coupling with zero source mass has zero marginals and zero plan. -/
theorem zeroMassFiniteCouplingPlan
    {X Y : Type*} [MeasurableSpace X] [MeasurableSpace Y]
    {mu : FiniteMeasure X} {nu : FiniteMeasure Y}
    (gamma : FiniteCoupling mu nu) (hZero : mu.mass = 0) :
    mu = 0 ∧ nu = 0 ∧ gamma.plan = 0 := by
  have hCommon := finiteCouplingMarginalMassEq gamma
  have hPlanMass := finiteCouplingPlanMassEqFirst gamma
  refine ⟨(FiniteMeasure.mass_zero_iff mu).mp hZero, ?_, ?_⟩
  · exact (FiniteMeasure.mass_zero_iff nu).mp
      (hCommon.symm.trans hZero)
  · exact (FiniteMeasure.mass_zero_iff gamma.plan).mp
      (hPlanMass.trans hZero)

/-- Finite coupling plans have the common marginal mass, are reconstructed
from their normalized probability plans, and normalization preserves weak
convergence when that mass is positive. -/
theorem fixedMassPlanNormalization
