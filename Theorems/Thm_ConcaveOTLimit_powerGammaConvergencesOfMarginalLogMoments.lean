import Theorems.Thm_ConcaveOTLimit_primaryPowerGammaConvergesOfMarginalLogMoments
...
/-- Positive exponents tending to zero give both the primary power Gamma
limit and the rescaled logarithmic Gamma limit on the distance-optimal
face. -/
theorem powerGammaConvergencesOfMarginalLogMoments
  ...
  have hPrimarySmall :=
    primaryPowerGammaConvergesOfMarginalLogMoments
      hMu hNu smallEpsilon hSmallPos hSmallLeHalf hSmallTendsto
  have hRescaledSmall :=
    rescaledPowerSequentialGammaConvergesOnDistanceOptimalFace
      hMu hNu smallEpsilon hSmallPos hSmallLeHalf hSmallTendsto
  have hPrimaryEventually :
      ∀ᶠ n in atTop,
        (fun gamma : FiniteCoupling mu nu =>
          (profileCost (powerProfile (smallEpsilon n)) gamma : EReal)) =
        (fun gamma : FiniteCoupling mu nu =>
          (profileCost (powerProfile (epsilon n)) gamma : EReal)) := by
  ...
  exact
    ⟨(sequentialGammaConvergesOn_congrEventually
        hPrimaryEventually).mp hPrimarySmall,
      (sequentialGammaConvergesOn_congrEventually
