...
/-- At a level whose interior hits are all locally strict crossings, a forward
arch in a full-mass monotone support ends at the unique consecutive right
endpoint of its source level. This is a local regular-level statement; it does
not assert global finiteness or alternation of level crossings. -/
theorem monotoneArchEndpointRigidity
