...
import Theorems.Thm_ConcaveOTLimit_strictForwardLexicographicSupportIsJuilletMonotone
...
/-- A forward lexicographic coupling of mutually singular marginals has a
full-mass support satisfying the interval-based monotone-arch conditions. -/
theorem existsFullMassMonotoneArchSupport
  ...
  obtain ⟨strictSupport, hStrictFull, hStrictForward, hStrictLex⟩ :=
  ...
  refine ⟨strictSupport, hStrictFull, ?_⟩
  intro p hp q hq
  rcases p with ⟨x, y⟩
  rcases q with ⟨x', y'⟩
  exact
