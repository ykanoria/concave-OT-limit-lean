...
private theorem existsSpanningCrossingCut
  ...
  obtain ⟨t, ht, hCut⟩ :=
    existsMonotoneArchSpanningCut S hLaminar hab
...
/-- Two forward couplings with the same marginals are equal when each is
carried by a laminar family of arches and the source marginal is atomless. -/
theorem finiteCoupling_eq_of_forward_laminarSupports
    {mu nu : FiniteMeasure Real}
    (hAtomless : IsAtomlessFinite mu)
    (gamma eta : FiniteCoupling mu nu)
    (hGammaForward : IsForwardPlan gamma)
    (hEtaForward : IsForwardPlan eta)
    (S R : Set (Real × Real))
    (hGammaFull : IsSupported gamma S)
    (hGammaLaminar :
      ∀ p ∈ S, ∀ q ∈ S, ArchesDoNotCross p q)
    (hEtaFull : IsSupported eta R)
    (hEtaLaminar :
      ∀ p ∈ R, ∀ q ∈ R, ArchesDoNotCross p q) :
    gamma = eta := by
  apply Subtype.ext
  ...
  · have hSpan :=
      spanningMeasureEq hAtomless gamma eta
        ...
        hGammaFull hGammaLaminar
        hEtaFull hEtaLaminar hab
...
/-- Juillet monotone-arch couplings of atomless, stochastically ordered
marginals are unique. Mutual singularity is not needed for uniqueness. -/
theorem monotoneArchPlan_unique
    {mu nu : FiniteMeasure Real}
    (hAtomless : IsAtomlessFinite mu)
  ...
  obtain ⟨_hGammaCoupling, S, _hSMeasurable, hSFull, hSMonotone⟩ :=
    hGamma
  obtain ⟨_hEtaCoupling, R, _hRMeasurable, hRFull, hRMonotone⟩ :=
    hEta
  exact
    finiteCoupling_eq_of_forward_laminarSupports
      hAtomless gamma eta hGammaForward hEtaForward S R
      hSFull (fun p hp q hq => (hSMonotone p hp q hq).1)
      hRFull (fun p hp q hq => (hRMonotone p hp q hq).1)
...
theorem oneDimensionalExcursionVariational
    (mu nu : FiniteMeasure Real)
  ...
  · intro gamma hGamma
    exact monotoneArchPlan_unique
      hAtomless hOrder gamma gammaEC hGamma hEC
